...
1. APK をダウンロードしてインストール
2. アプリを起動 → 「通知アクセスを許可する」をタップ
3. システム設定で **Shazam検索ショートカット** を ON にする

### 「制限付きのアプリ」と表示される場合（Android 13+）

Play Store 以外からインストールした APK では、通知アクセスの許可時に「制限付きのアプリ」と表示されることがあります。

**設定 → アプリ → アプリを管理 → Shazam検索ショートカット → ⋮（三点メニュー）→「制限付き設定を許可」**

その後、通知アクセスの設定画面に戻ると ON にできます。
